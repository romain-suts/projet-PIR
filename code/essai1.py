...
for i in range(len(distr_temp)):
    distr_temp[i] = distr_temp[i] - distr_temp[4999] + 110 #ajustement pour ne pas descendre en dessous de 0K

# CALCUL DISTRIBUTION TEMPERATURE ANALYTIQUEMENT
temp_analytique = np.zeros(5000)
temp_analytique[4999] = 110 # température de surface de Ganymède
temp_analytique[0] = 2.07748e+08 # température centrale

for i in range (1,len(distr_rayon)-1):
    if distr_rayon[i] < rayon_silicate :
        temp_analytique[i] = 110 - 0.002*rayon/k_silicate + 0.002*rayon**2/(k_silicate*distr_rayon[i])
    else : 
        temp_analytique[i] = 110 - 0.002*rayon/k_glace + 0.002*rayon**2/(k_glace*distr_rayon[i])

    
    
    
plt.figure(6)
# plt.plot(distr_rayon*10**(-3), distr_temp, label='Résolution numérique')
plt.plot(distr_rayon, temp_analytique, label='Résolution analytique')
plt.xlabel('Rayon (km)')
...
plt.figure(7)
plt.plot(distr_temp, distr_pression)
plt.xlabel('Température (K)')
plt.ylabel('Pression (Pa)')
plt.yscale('log')
plt.title('Diagramme de phase de Ganymède')
plt.grid()
plt.show()
